...
class FormateurRepository extends ServiceEntityRepository
{
    ...
    */
    public function findFormateurByid($numtel){
        return $this->createQueryBuilder('formateur')
            ->where('formateur.id LIKE :id OR formateur.nom LIKE :id OR formateur.prenom LIKE :id OR formateur.email LIKE :id OR formateur.numtel LIKE :id' )
            ->setParameter('id', '%'.$numtel.'%')
            ->getQuery()
            ->getResult();
    }
